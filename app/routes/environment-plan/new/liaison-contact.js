...
export default Ember.Route.extend(ResetScroll, {
  ...
  model() {
    ...
    const promise = new Ember.RSVP.Promise(function(resolve) {
      ...
      const recordNotFound = () => {
        const contact = store.createRecord('liaison-contact', {id: 'ltdm0'});
      ...
      store.findRecord('liaison-contact', 'ltdm0').then(recordFound, recordNotFound);
    });
  ...
  _notifyListeners() {
    this.get('submissionStatus').leaving('liaison-contact', this.get('currentModel').get('hasErrors'));
  },
  actions: {
